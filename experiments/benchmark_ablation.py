...
def main():
    ...
    vl = VelmFull(
        ...
        embed_dim=128,
        latent_dim=128,
        state_dim=96, # Maintained budget constraints
        num_latent_thoughts=1,
    )
    models["velm_lite"] = vl
    model_info["velm_lite"] = count_params(vl)

    vl_no_thoughts = VelmFull(
        vocab_size=vocab_size,
        block_size=args.block_size,
        embed_dim=128,
        latent_dim=128,
    ...
    models["velm_no_thoughts"] = vl_no_thoughts
    model_info["velm_no_thoughts"] = count_params(vl_no_thoughts)
    ...
    models["velm_no_cib"] = vl_no_cib
    model_info["velm_no_cib"] = count_params(vl_no_cib)
    ...
    for name, model in models.items():
        ...
        for iter_num in range(args.max_iters):
            ...
            if name.startswith("velm"):
                ...
                loss = F.cross_entropy(logits.reshape(-1, logits.size(-1)), Y.view(-1))
                loss = loss + (1e-3 * cib_loss if name != "velm_no_cib" else 0.0)
            else:
                logits = model(X)
                loss = F.cross_entropy(logits.view(-1, logits.size(-1)), Y.view(-1))
