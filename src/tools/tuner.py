...
def train_and_eval(cfg: TunerConfig, hparams: dict) -> tuple[float, int]:
    ...
    device = torch.device("cuda" if (cfg.device == "auto" and torch.cuda.is_available()) or cfg.device == "cuda" else "cpu")

    model = VelmFull(vocab_size=vocab, block_size=hparams["block_size"], embed_dim=hparams["embed_dim"], latent_dim=hparams["latent_dim"], state_dim=hparams["state_dim"]).to(device)
...
def ui_thread_fn(study: optuna.study.Study, stop_event: threading.Event, cfg: TunerConfig, refresh: float = 0.5):
        ...
        while not stop_event.is_set():
            ...
            if len(hp_rows) >= 2:
                try:
                        ...
                        if cfg.use_umap:
                            ...
                            with warnings.catch_warnings():
                                ...
                                try:
                                    ...
                                    n_samples = X.shape[0]
                                    if n_samples < 3:
                                        raise RuntimeError("Too few samples for UMAP; falling back to PCA")

                                    n_neighbors = min(15, max(2, n_samples - 1))
                                    # configure UMAP with conservative defaults to avoid warnings on small data
                                    reducer = UMAP(n_components=2, random_state=cfg.seed, n_neighbors=n_neighbors, min_dist=0.1, metric="euclidean", verbose=False)
                                    ...
                                    warnings_log.append(f"UMAP disabled due to import/fit error: {e_umap}")
                                    Xc = X - X.mean(axis=0)
                    ...
                    for idx in order[:3]:
                        plt.annotate(str(hp_labels[idx]), (xs[idx], ys[idx]), fontsize=8, weight="bold")
                    ...
                    try:
                        ...
                        try:
                            ...
                            for i_pt in range(len(xs)):
                                ...
                                color_idx = int(norm_vals[i_pt] * (len(cmap) - 1)) if len(norm_vals) == len(xs) else 0
                            ...
                            for idx in order[:3]:
                                ...
                                legend.append("■ ", style=f"{cmap[int(norm_vals[idx]*(len(cmap)-1))]}")
            ...
            for i, t in enumerate(sorted_trials[:min(20, len(sorted_trials))]):
                ...
                table.add_row(
                    ...
                    f"{params.get('lr', '-'):.4g}" if isinstance(params.get('lr', None), float) else str(params.get('lr', '-')),
                    f"{params.get('cib_lambda', '-'):.2g}" if isinstance(params.get('cib_lambda', None), float) else str(params.get('cib_lambda', '-')),
                    str(t.state),
                )
            ...
            left_col.split_column(Layout(Panel(table, title="Top Trials"), ratio=3), Layout(name="plot", ratio=2))
            ...
            layout["bottom"].split_row(Layout(Panel(prog, title="Progress")), Layout(Panel(warnings_table, title="Captured Warnings")))
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--weight-param-millions", type=float, default=None, help="Penalty weight per million parameters (overrides default in config). Lower prefers accuracy over compactness.")
    parser.add_argument("--no-umap", action="store_true", help="Disable UMAP and force PCA-only 2D projection (suppresses umap/numba warnings).")
    args = parser.parse_args(argv)

    # Allow CLI override for the parameter-count penalty
    penalty = args.weight_param_millions if args.weight_param_millions is not None else TunerConfig.weight_param_millions
    cfg = TunerConfig(trials=args.trials, steps=args.steps, device=args.device, out=args.out, seed=args.seed, weight_param_millions=penalty, use_umap=not args.no_umap)
    ...
    try:
        try:
            ...
            msg = str(e)
            if "CategoricalDistribution does not support dynamic value space" in msg or "dynamic value space" in msg:
                console.print("Incompatible prior study distributions detected — creating a fresh study to continue tuning.")
                ts = int(time.time())
                new_db = os.path.join(cfg.out, f"optuna_{ts}.db")
                new_storage = f"sqlite:///{new_db}"
                study_name = f"velm_tuning_{ts}"
                study = optuna.create_study(direction="maximize", study_name=study_name, storage=new_storage, load_if_exists=False)
